...
async function fixAllVendors() {
  try {
    ...
    const vendorsWithoutCompany: any[] = [];

    for (const vendor of allVendors) {
      const vendorCompany = await models.VendorCompany.findOne({
        where: { vendorId: (vendor as any).id },
      });
    ...
    vendorsWithoutCompany.forEach((vendor: any, index: number) => {
    ...
    for (const vendor of vendorsWithoutCompany) {
      try {
        console.log(`${colors.blue}→${colors.reset} Fixing vendor ID ${colors.cyan}${(vendor as any).id}${colors.reset} (${(vendor as any).email})...`);
        ...
        console.log(`  ${colors.red}✗${colors.reset} Error: ${(error as Error).message}\n`);
        errorCount++;
      }
    }
