...
export const sendMessage = async (req, res, next) => {
    try {
        ...
        if (!userId) {
            return res.status(401).json({ error: "User not authenticated. Please log in to use chat." });
        }
...
export const checkLLMHealth = async (req, res, next) => {
    try {
...
export const getSessions = async (req, res, next) => {
    try {
        ...
        const sessions = await chatService.getSessions(userId, negotiationId || null);
...
export const testDatabaseConnection = async (req, res, next) => {
    try {
        ...
        if (negotiationId) {
            const context = await contextService.getNegotiationContext(negotiationId);
            ...
            const context = await contextService.getRequisitionContext(requisitionId);
            testResults.contextData = {
                type: "requisition",
                requisitionId,
                data: context,
